...
`panels/templates/*.studio.json` 이 **실제 페이지까지** 이어지는지 게이트한다. "템플릿에서 시작하기"가
빈 화면을 낳지 않는다는 보증이다.

```bash
npm run p9    # 템플릿 → lift → 위젯 규격 → emitPage. 통과 = 규격 오류 0 · 미지원 위젯 0
```

## P10 — 광고 포맷 팩 (제휴형 지면 구조화)

키즈노트 광고 상품 소개서의 **"맞춤 제휴 광고 — 별도 페이지 구성"** 을 패널 화면 부품으로 구조화했다.
DA 노출형(스플래시·메인배너·종료팝업)은 **앱 셸 소유라 여기 없다** — 전면형만 패널 안에서 재현한다.

**위젯 7종** (전부 비-DP. 광고는 기기 상태가 아니라 캠페인 데이터를 그린다):

| 위젯 | 종류 | 쓰는 곳 |
|---|---|---|
| `adOfferRow` | link | 혜택 탭 — 여러 광고주 리스트 |
| `adProductCard` | link | 브랜드위크 — 상품 진열 |
| `adStepGuide` | text[] | 참여 절차 (번호 자동) |
| `adFactRow` | text | 모집 인원·기간 등 캠페인 정보 |
| `adCoupon` | text | 쿠폰 코드 강조 |
| `adConsent` | text | **개인정보 수집·활용 고지** |
| `adDismissBar` | text | 전면 광고 닫기 줄 |

**템플릿 7종**: 리드수집형 · 체험단형 · 리서치형 · 혜택쿠폰형 · 브랜드위크형 · 혜택탭형 · 전면광고형.

- `adConsent` 는 장식이 아니다. 리드수집·체험단은 **동의 고지 없이 집행 불가**라(키즈노트 집행정책 19p 와 동형)
  템플릿이 그 자리를 비워 두지 않고 문구 초안째 들고 시작한다.
- 광고는 **비주얼이 곧 상품**이라 클래스만 뱉으면 빈 페이지가 된다. 광고 위젯을 쓰는 페이지에만
  `index.less` 를 함께 낸다(`emitAdStyles`). 색은 `panel.theme` 에서 온다 — 팔레트를 새로 만들지 않는다.
  공용 스캐폴드 `app.less` 는 건드리지 않는다(기기 패널에 광고 스타일이 섞이면 안 된다).

**실빌드 검증**: 광고 7종 + 기존 기기 4종 전부 `ray build -t tuya` 통과. 컴파일된 CSS 에
테마 색(`#00a872`)이 그대로 실리고 클래스가 해시되지 않아 실제로 적용되는 것까지 확인했다.

> **이것은 광고 서빙이 아니라 광고 퍼블리싱이다.** 소재는 저작 시점에 번들에 굽힌다 —
> 교체하려면 재빌드·재심사가 필요하고, 노출·클릭 집계도 없다. 런타임 슬롯(`AdSlot`)으로 가는
> 구조 변경은 별건이며, Tuya 미니앱 심사의 원격 콘텐츠 정책 확인이 선행 조건이다.
> 검토서: `add-ads/FEASIBILITY.md`.
...
- **P9 ✔** — 템플릿 왕복 증명(`src/p9.mjs`) — "템플릿에서 시작하기"가 실제 페이지로 이어지는지 게이트.
- **P10 ✔** — 광고 포맷 팩. 키즈노트 제휴형 지면을 위젯 7종·템플릿 7종으로 구조화. 아래 "P10" 절.
